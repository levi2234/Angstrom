...
def test_subtle_motion_amplification():
    """
    Test motion amplification on the subtle motion video.
    """
    ...
    if not os.path.exists(test_video_path):
        ...
        print("Please run generate_subtle_test_video.py first to create the test video.")
        return

    print("Testing motion amplification on subtle motion video...")
    ...
    # Apply motion amplification with parameters suited for subtle motion
    print("Applying motion amplification...")
    amplification_factor = 50  # Higher amplification for subtle motion

    # Target the specific frequency of the motion (2 Hz) with a narrow band
    amplified_video = amplifier.amplify(
        amplification_factor=amplification_factor,
        frequency_range=(1.8, 2.2)  # Hz - narrow band around 2 Hz motion
    )

    # Save the amplified video
    output_path = "test_subtle_motion_amplified.mp4"
    ...
    print(f"Frequency range: 1.8-2.2 Hz (targeting 2 Hz motion)")
...
if __name__ == "__main__":
    test_subtle_motion_amplification()